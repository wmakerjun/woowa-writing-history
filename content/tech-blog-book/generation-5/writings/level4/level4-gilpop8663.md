...
source: "https://github.com/woowacourse/woowa-writing-5/blob/gilpop8663/LEVEL_4.md"
source_path: "LEVEL_4.md"
---
...
다른 사람의 코드를 많이 보고 배우고, GPT에 특정 동작을 하는 함수 혹은 변수의 이름을 추천해달라고 하여 가독성을 높이는 방법이 있다.
...
코드에서만 TDD를 적용하는 것이 아니라 일상에서도 적용할 수 있다. 하루 일정을 미리 시작하기 전 정해놓거나, 앞으로 한 시간 안에 무엇을 하겠다고 생각을 하면 현재 내가 무엇을 해야 할지 명확히 알고, 그 목표를 달성한 지 여부 또한 피드백할 수 있다.
...
문제를 다르게 바라보는 시각은 책을 읽었을 때 넓어진다고 생각한다. 그리고 비슷한 일을 반복해서 할 때 항상 같은 방식으로 해결하지 않고 의도적으로 다른 방식으로 해결하는 것을 연습하는 방법이 있다.

### 비즈니스 가치

한 기능을 구현하는 동안은 다른 기능을 구현할 시간은 멈추게 된다. 비즈니스 가치를 크게 높이는 기능들이 무엇인지 파악하고 우선순위를 두어 투입한 시간 대비 얻는 것이 많은 것부터 구현할 줄 아는 개발자가 되고 싶다.

비즈니스 가치의 순위를 매기는 방법으로는 회사의 목표가 무엇인지를 염두에 두고 회사에 가장 큰 이득을 가져다주는 방법을 생각하면 된다. 예를 들어 개발에 사용하는 기술의 버전을 최신화하는 이슈와 새로운 기능을 구현하는 이슈 중 회사에 이득을 가져다주는 방법은 새로운 기능을 구현하는 일이다. 하지만 개발에 사용하는 기술을 오랫동안 업데이트하지 않는다면 낡은 회사가 되어 개발자들이 오기 싫어하는 회사가 된다. 그러면 회사가 추구하는 목표를 이루는 데 방해가 되기 때문에 적절한 타이밍에 기술을 업데이트하는 것이 중요하다.
...
우아한테크코스에 들어오기 전에는 웹 접근성에 대해서 잘 몰랐던 “나"였다. 내가 최종적으로 상상하는 개발자는 다양한 사람들이 동등하게 사이트를 이용할 수 있도록 웹 접근성을 잘 지켜서 개발하는 것이다. 투표 SNS 커뮤니티 프로젝트를 하면서 많이 배우게 되었는데 스크린 리더기를 사용하는 사용자는 키보드나 휴대전화를 사용하지, 마우스를 사용하지 않는다는 사실을 깨우치게 된 점이 가장 큰 수확이라고 생각이 든다. 실제로 눈을 감고 이용할 수 있는지 확인해 보라는 제임스의 말처럼 이를 실천하는 개발자가 되고 싶다.
...
그리고 회사 입장에서도 기록 잘하는 구성원이 중요한데, 해당 코드를 작성한 구성원이 이직했을 때 코드에 대한 설명이 없다면 코드를 유지보수하는 사람 입장에서는 설명 없이 코드를 이해하는 데 많은 시간이 들게 된다. 만약 퇴사한 팀원이 코드에 대해 정리해 둔 글이 있다면 앞선 상황을 방지할 수 있어서 기록하는 습관은 중요하다.
